...
@app.route("/ask/packages/<project>", methods=["POST"])
def ask_packages(project: str):
    create_graph_if_empty(project)
    try:
...
@app.route("/ask/function/<project>/<className>", methods=["POST"])
def ask_class(project: str, className: str):
    create_graph_if_empty(project)
    try:
        # Get the JSON data from the request body
        ...
        if 'data' in data:
            ...
            # Return a success response
            return ai_query_with_graph(create_function_graph(complete_graphs[project], className).model_dump_json(), messages, current_level)
        else:
            # If 'data' key is missing or doesn't contain a list, return an error response
            return jsonify({'error': 'Invalid data format'}), 400
...
@app.route("/ask/file-and-classes/<project>/<package>", methods=["POST"])
def ask_package(project: str, package: str):
    ...
    try:
        ...
        if 'data' in data:
            ...
            # Return a success response
            return ai_query_with_graph(create_files_classes_graphs(complete_graphs[project], package).model_dump_json(), messages, current_level)
        else:
            # If 'data' key is missing or doesn't contain a list, return an error response
            return jsonify({'error': 'Invalid data format'}), 400
